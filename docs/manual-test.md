...
- 作業ディレクトリ: `D:\AI\WindowsApp\encode-publish-metadata-evidence-queue`
...
```powershell
cd D:\AI\WindowsApp\encode-publish-metadata-evidence-queue
npm test
npm start
```
...
5. Windows-first CLI + queue manifest の UI または配布面を開き、検証結果をユーザーが次アクションに移せる粒度で確認する。
...
- docs ZIP が `dist/encode-publish-metadata-evidence-queue-docs.zip` に存在する。
